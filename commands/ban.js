module.exports = {
	...
	async execute(message, args, client, sleep, config, Client, Discord) {
		...
		const Embed = new Discord.MessageEmbed().setColor(Math.round(Math.random() * 16777215)).setTitle(`Banned ${user.mention} for ${args.join(' ').replace(`${args[0]} `, '')}`);
		...
		try {
			...
			message.channel.send(e.split('at')[0]);
		}
	},
};